                ...
                <div class="col-md-6">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">{{isset($row->id) ? "Update Category" : "Add Category"}}</h4>
                        </div>
                        <div class="card-body">
                            <form action="{{ route('admin.category.post') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-control" id="parent_category" name="parent_category">
                                        ...
                                        @foreach($categories as $cate)
                                            <option value="{{$cate->id}}" {{ (isset($row->p_category) && $cate->id == $row->p_category) ?'selected':'' }}>{{$cate->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="file" class="dropify" data-default-file="{{ isset($row->image)? url('/storage/category/'.$row->id.'/'.$row->image) : ''}}" name="image" />
                                    @error('image')
                                    <div class="text-danger">
                                        <small>{{ $message }}</small>
                                    </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="file" class="dropify" data-default-file="{{ isset($row->dark_image)? url('/storage/category/'.$row->id.'/'.$row->dark_image) : ''}}" name="dark_image" />
                                    @error('dark_image')
                                    <div class="text-danger">
                                        <small>{{ $message }}</small>
                                    </div>
